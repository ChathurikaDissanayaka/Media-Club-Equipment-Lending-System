...
<body>
	...
	<div id="main">
	  ...
	  <div class="container">
	  	<h2 style=" text-align: center">Set Current Event</h2>
	  	<div class="updateID">
		...
		if(isset($_POST['submit1']))
		{
			...
			$sql = "SELECT * from `crew member` where ID like '%$_POST[id]%' ";
	...
	if(isset($_POST['submit2']))
	{
		...
		$checkEve = "SELECT * FROM `event` WHERE `Event Name` ='$event'";
		$resEve = mysqli_query($db, $checkEve);

		
			if($event == ""){
				$setNull = "CALL `set_current_event_null`('$idnum')";
				mysqli_query($db, $setNull);
				?>
				 
				<script type="text/javascript">
				    alert("Current event is set!");
				</script>
				<?php
			}
			elseif(mysqli_num_rows($resEve) == 1){
				$update = "UPDATE `crew member` SET `Current Event` = '$event' WHERE `crew member`.`ID` = '$idnum'";
